...
if ( ! class_exists( 'BP_Private_Avatar' ) ) :
	...
	class BP_Private_Avatar {
		...
		public function __construct() {
			...
			add_action( 'bp_before_profile_avatar_upload_content', array( $this, 'add_profile_avatar_setting' ) );
		...
		<script>
			jQuery(document).ready(function($) {
				jQuery(document).on("change", "#sp_make_avatar_private", function(){
					var obj = jQuery(this);
					var value = obj.val();
					jQuery.post( "<?php echo admin_url( 'admin-ajax.php' ); ?>", { action: "sp_private_avatar", setting: value })
					  .done(function( data ) {
		...
		public function sp_set_private_avatar() {
			...
			if ( ! empty( $_POST['setting'] ) ) {
				update_user_meta( bp_displayed_user_id(), '_make_avatar_private', (int) $_POST['setting'] );
				$return['option']	= 1;
			} else {
